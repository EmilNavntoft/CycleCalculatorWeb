...
namespace CycleCalculatorWeb.CycleModel.Solver
{
    public static class Solver
    {
        ...
        public static List<CycleComponent> Layout1()
        {
			...
			{
				OutletTemperature = Temperature.FromDegreesCelsius(30)
			};
			TemperatureBoundary tb2 = new TemperatureBoundary("TB2")
			{
				OutletTemperature = Temperature.FromDegreesCelsius(-8)
			};
        ...
        public static List<CycleComponent> Layout2()
        {
			...
			TemperatureBoundary tb2 = new TemperatureBoundary("TB2")
			{
				OutletTemperature = Temperature.FromDegreesCelsius(-6)
			};
			TemperatureBoundary tb3 = new TemperatureBoundary("TB3")
			{
				OutletTemperature = Temperature.FromDegreesCelsius(-25)
			};
			PRV PRV1 = new PRV("PRV1")
			{
        ...
        private static int SetIdents(List<CycleComponent> components)
        {
            int ident = 0;
            foreach (var component in components)
            {
                foreach (var port in component.Ports.Values)
                {
                    port.MdotIdent = ident++;
                    port.PIdent = ident++;
                }
            }
        ...
        private static void SetSolution(double[] solution, List<CycleComponent> components)
        {
            for (int i = 0; i < solution.Length; i++)
            {
                foreach (var component in components)
                {
                    foreach (var port in component.Ports.Values)
                    {
                        if (port.MdotIdent == i)
                        {
                            port.MassFlow = MassFlow.FromKilogramPerSecond(solution[i]);
                        }
                        if (port.PIdent == i)
                        {
                            port.Pressure = Pressure.FromPascal(solution[i]);
                        }
                    }
                }
            }
